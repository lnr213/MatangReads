...
    // Only allow deletion if the status is pending and it belongs to the current user
...
    JOIN books b ON br.book_id=b.book_id  <!-- FIXED: Changed b.bookid to b.book_id -->
    WHERE br.user_id = ? 
...
        // We use the calculate_fine function from config.php
        $fine = calculate_fine($r['due_date']);
        $total_fine += $fine;
    }
}

// 4. Calculate Net Amount Due (Fine minus Credit)
...
<!doctype html>
<html>
<head><meta charset="utf-8"><title>Dashboard - MatangReads</title>
...
<div class="container">
  <h2 style="color: #2d0115;">Welcome back, <?php echo htmlspecialchars($username); ?></h2>
...
        &nbsp;&nbsp;|&nbsp;&nbsp;
        <span style="color: #2d0115; font-weight: bold;"><i class="fas fa-coins"></i> Daily Fine Rate:</span> 
...
    <h3 style="color: #2d0115; border-bottom: 2px solid #AE8625; padding-bottom: 5px; margin-bottom: 15px;">Your Borrowing Activity</h3>
...
                // Define Action Link for Pending requests
                if ($status === 'pending') {
...
                <td><?php echo $action_link; ?></td> <!-- Display Action -->
            </tr>
            <?php endforeach; ?>
            
            <tr>
...
                <td style="color:<?php echo $net_amount_due > 0 ? 'red' : 'green'; ?>; font-weight:bold; background:#cceeff;">RM <?php echo number_format($net_amount_due, 2); ?></td>
...
            <?php echo $net_amount_due > 0 ? 'Pay Outstanding Fees' : 'Make Optional Payment'; ?> (RM <?php echo number_format($net_amount_due, 2); ?>)
        </a>
    </div>
    <?php endif; ?>
  </section>
...
    <h3 style="color: #2d0115; border-bottom: 2px solid #AE8625; padding-bottom: 5px; margin-bottom: 15px;">Featured Books</h3>
    <div class="books-box">
